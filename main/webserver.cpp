...
static esp_err_t gearbox_page_handler(httpd_req_t *req) {
    httpd_resp_send(req, P_gearbox, HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}

static esp_err_t get_gearbox_handler(httpd_req_t *req) {
    httpd_resp_set_type(req, "text/json");
    httpd_resp_sendstr(req, webGearboxJson().c_str());
    return ESP_OK;
}

static esp_err_t set_gearbox_handler(httpd_req_t *req) {
    webGearboxSet(req);
    httpd_resp_set_type(req, "text/json");
    httpd_resp_sendstr(req, "{\"state\":\"ok\"}");
    return ESP_OK;
...
esp_err_t api_brake_handler(httpd_req_t *req) {
...
esp_err_t start_web_server() {
    ...
    httpd_uri_t api_brake_uri = {
        .uri       = "/api/brake",
        .method    = HTTP_GET,
        .handler   = api_brake_handler,
        .user_ctx  = NULL
    };

    httpd_register_uri_handler(server, &api_brake_uri);
    ...
    httpd_uri_t gearboxPage = {
        .uri = "/gearbox.html",
        .method = HTTP_GET,
        .handler = gearbox_page_handler,
        .user_ctx = NULL
    };
    httpd_register_uri_handler(server, &gearboxPage);

    httpd_uri_t getGearbox = {
        .uri = "/web/gearbox.json",
        .method = HTTP_GET,
        .handler = get_gearbox_handler,
        .user_ctx = NULL
    };
    httpd_register_uri_handler(server, &getGearbox);

    httpd_uri_t setGearbox = {
        .uri = "/set/gearbox",
        .method = HTTP_GET,
        .handler = set_gearbox_handler,
        .user_ctx = NULL
    };
    httpd_register_uri_handler(server, &setGearbox);
